...
class Main(CustomerPortal):
    ...
    @http.route(['/my/builds', '/my/builds/page/<int:page>'], type='http', auth="user", website=True)
    def portal_my_projects(self, page=1, sortby=None, **kw):
        ...
        builds = Build.search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
    ...
    def portal_my_project(self, build_id=None, access_token=None, **kw):
        try:
        ...
        values = self._get_page_view_values(build_sudo, access_token, values, None, False, **kw)
